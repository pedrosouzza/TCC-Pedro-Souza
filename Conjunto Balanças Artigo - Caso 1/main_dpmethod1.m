...
for i = 1:n
    soma = vpa ((mte_dpmethod1(i) - mt(i))^2);
end

EM_dpmethod1 = vpa(1/n * sqrt(soma));
...
workbookName = 'Pressao_e_massa_input_dpmethod1.xlsx';

% Defina os dados que você deseja exportar
inputData = table(p', mte_dpmethod1', mt', 'VariableNames', {'Pressao (MPa)', 'Massa Balança Teste Estimada (Kg)', 'Massa Balança Teste Original (kg)'});
inputData_transposta = table(p, mte_dpmethod1, mt, 'VariableNames', {'Pressao (MPa)', 'Massa Balança Teste (Kg)', 'Massa Balança Teste Original (kg)'});
resultsData = table(A0te_dpmethod1, A0t, lamte_dpmethod1, lamt, EM_dpmethod1, 'VariableNames', {'Área Efetiva da Balança Teste Estimada (mm²)', 'Área Efetiva da Balança Teste Original (mm²)' 'Coef. Distorção da Balança Teste Estimado (1/MPa)', 'Coef. Distorção da Balança Teste Original (1/MPa)', 'Erro nas massas (Kg)'});
...
coeficientes3 = polyfit(p, mte_dpmethod1, 1);
...
R2__ = 1 - sum((mte_dpmethod1 - yFit3).^2) / sum((mte_dpmethod1 - mean(mte_dpmethod1)).^2);
...
subplot(3,1,2)
i = plot(p, mte_dpmethod1, 'x', p, yFit3);
grid
