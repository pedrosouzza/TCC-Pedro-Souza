...
for i = 1:n
    soma = vpa ((me_wllspe(i) - m (i))^2);
end

EM_wllspe = vpa(1/n * sqrt(soma));
...
workbookName = 'Pressao_e_massa_input_wllspe.xlsx';

% Defina os dados que você deseja exportar
inputData = table(p', m', me_wllspe', 'VariableNames', {'Pressao (MPa)', 'Massa Original (Kg)', 'Massa Estimada (Kg)'});
inputData_transposta = table(p, m, me_wllspe, 'VariableNames', {'Pressao (MPa)', 'Massa Original (Kg)', 'Massa Estimada (Kg)'});
resultsData = table(A0original, A0_wllspe, lambdaoriginal, lambda_wllspe, EM_wllspe, 'VariableNames', {'Área Efetiva Original (mm²)','Área Efetiva Estimada (mm²)', 'Coef. Distorção Original (1/MPa)', 'Coef. Distorção Estimado (1/MPa)', 'Erro na massa (Kg)'});
...
coeficientes1 = polyfit(p, me_wllspe, 1);
...
R2_ = 1 - sum((me_wllspe - yFit1).^2) / sum((me_wllspe - mean(me_wllspe)).^2);
