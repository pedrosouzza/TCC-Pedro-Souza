...
for i = 1:n
    soma = vpa ((mte_pmethod0(i) - mt(i))^2);
end

EM_pmethod0 = vpa(1/n * sqrt(soma));
...
workbookName = 'Pressao_e_massa_input_pmethod0.xlsx';

% Defina os dados que você deseja exportar
inputData = table(p', mte_pmethod0', mt', 'VariableNames', {'Pressao (MPa)', 'Massa Balança Teste Estimada (Kg)', 'Massa Balança Teste Original (kg)'});
inputData_transposta = table(p, mte_pmethod0, mt, 'VariableNames', {'Pressao (MPa)', 'Massa Balança Teste (Kg)', 'Massa Balança Teste Original (kg)'});
resultsData = table(A0te_pmethod0, A0t, lamte_pmethod0, lamt, EM_pmethod0, 'VariableNames', {'Área Efetiva da Balança Teste Estimada (mm²)', 'Área Efetiva da Balança Teste Original (mm²)' 'Coef. Distorção da Balança Teste Estimado (1/MPa)', 'Coef. Distorção da Balança Teste Original (1/MPa)', 'Erro nas massas (Kg)'});
...
coeficientes3 = polyfit(p, mte_pmethod0, 1);
...
R2_ = 1 - sum((ms - yFit2).^2) / sum((ms - mean(ms)).^2);
R2__ = 1 - sum((mte_pmethod0 - yFit3).^2) / sum((mte_pmethod0 - mean(mte_pmethod0)).^2);
...
% Plota os valores de pressão e carga utilizados como input da balança teste
figure(1)
subplot(3,1,1)
h = plot(p, mt, 'x', p, yFit);
grid
title('Relação massa em função da pressão da balança teste input')
xlabel('Pressão (MPa)')
ylabel('Massa (Kg)')
...
subplot(3,1,2)
i = plot(p, mte_pmethod0, 'x', p, yFit3);
grid
...
grid
title('Relação massa em função da pressão da balança padrão output')
xlabel('Pressão (MPa)')
ylabel('Massa (Kg)')
