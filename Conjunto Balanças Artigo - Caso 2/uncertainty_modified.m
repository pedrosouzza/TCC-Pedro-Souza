...
p_PA_modificada = [12000000; 15000000; 18000000; 21000000; 24000000; 27000000; 30000000; 33000000; 36000000; 39000000; 42000000; 45000000; 48000000; 51000000; 54000000; 57000000; 60000000]; % Range de pressão da balança de teste (Pa)
...
mr = 12.3287407; % Somatório das massas do sistema - Balança Teste (Kg)
...
for i = 1:length(p_PA_modificada)
    ...
    A0(i) = ((mr * (1 - rhoa/rhomr)) * g + (sigma * C)) /((p_PA_modificada(i) + (rhof - rhoa) * g * deltah) * (1 + (alphac + alphap) * (t - tr)));
...
A0_Unica_modificada = 0.00000402977001005089; % Área Efetiva da balança teste do artigo (m²)
...
lambda_Unica_modificada = 3.99983371084161; % Coef. Distorção da balança teste do artigo (1/Pa)
