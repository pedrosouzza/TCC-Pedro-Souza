...
% Chama a função pmethod1_temp
[A0te_pmethod1_vec, lamte_pmethod1_vec, mte_pmethod1] = Pmethod1_temp(p, ts, A0s, lams, Cs, alps, tt, mt_vec, ms_vec, Ct, alpt, g);

% Criação de vetores
metodo1 = repmat('Pmethod1', length(t),1);
metodo2 = repmat('Pmethod0', length(t),1);

% Nome da pasta de trabalho no Excel
workbookName = 'Avaliação Temperatura - Caso 4.xlsx';
   
% Defina os dados que você deseja exportar
Valores1 = table(metodo1, t, A0te_pmethod1_vec, lamte_pmethod1_vec, 'VariableNames', {'Método', 'Temperatura (ºC)','A0 (mm²)', 'Coef. Distorção (1/MPa)'});
...
writetable(Valores1, workbookName, 'Sheet', 'Pmethod1');
