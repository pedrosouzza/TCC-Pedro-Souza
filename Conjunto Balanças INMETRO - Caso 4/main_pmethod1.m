...
for i = 1:n
    soma = vpa ((mte_pmethod1(i) - mt(i))^2);
end

EM_pmethod1 = vpa(1/n * sqrt(soma));
...
workbookName = 'Pressao_e_massa_input_pmethod1.xlsx';

% Defina os dados que você deseja exportar
inputData = table(p', mte_pmethod1', mt', 'VariableNames', {'Pressao (MPa)', 'Massa Balança Teste Estimada (Kg)', 'Massa Balança Teste Original (kg)'});
inputData_transposta = table(p, mte_pmethod1, mt, 'VariableNames', {'Pressao (MPa)', 'Massa Balança Teste (Kg)', 'Massa Balança Teste Original (kg)'});
resultsData = table(A0te_pmethod1, A0t, lamte_pmethod1, lamt, EM_pmethod1, 'VariableNames', {'Área Efetiva da Balança Teste Estimada (mm²)', 'Área Efetiva da Balança Teste Original (mm²)' 'Coef. Distorção da Balança Teste Estimado (1/MPa)', 'Coef. Distorção da Balança Teste Original (1/MPa)', 'Erro nas massas (Kg)'});
...
coeficientes3 = polyfit(p, mte_pmethod1, 1);
...
R2__ = 1 - sum((mte_pmethod1 - yFit3).^2) / sum((mte_pmethod1 - mean(mte_pmethod1)).^2);
...
subplot(3,1,2)
i = plot(p, mte_pmethod1, 'x', p, yFit3);
grid
